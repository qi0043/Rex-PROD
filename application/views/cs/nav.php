...
<head>
	<title>Flinders University - RHD Research Excellence</title>
    <meta charset="utf-8">
...
<?php
    $ci =& get_instance();
    $ci->load->config('rex');
    $rex_url = $ci->config->item('institute_url');
    $flextra_url = $ci->config->item('flextra_url');
?>
...
<script type="text/javascript">
	...
	$(document).ready(function() 
	{
			...
			posting.done(function(data, status) {
				if(status == 'success')
				{
					...
					if(resultobj.status == 'success')
					{
                        ...
                        var rex_url = "<?php echo $rex_url; ?>";
                        var redirect_url = rex_url + 'items/' + uuid + '/' + version + '/?token=' + token;
						location.reload();
		...
		$('.create_examination').click(function(e) {
            $('.create_examination').prop('disabled', true);
			var data = $.parseJSON($(this).attr('data-button'));
			//alert(data.stu_id);
		...
		$('.milestone-link').click(function(e) {
            ...
            var flextra_url = "<?php echo $flextra_url; ?>";
            var newURL = flextra_url + uuid+'/'+ version;
			window.open(newURL);
		});
...
<nav class="navbar navbar-inverse navbar-fixed-top">
    <div id="nav-banner">
        <div class="navbar-header">
            <a class="navbar-brand" href="<?php echo base_url('cs/main'); ?>">RHD Research Excellence <?php echo isset($current_view)? '&nbsp;: :&nbsp;'.$current_view : ''?></a>
        </div>
                    ...
                    <ul class="nav navbar-nav navbar-center role-desktop">
                        <li class="dropdown">
                            ...
                            <ul class="dropdown-menu">
                                ...
                                <li><a href="<?php echo base_url('cs/student'). '/'. $this->session->userdata['fan'] .'/'; ?>">Student</a></li>
                                <li><a href="<?php echo base_url('cs/supervisor'). '/'. $this->session->userdata['fan'] .'/'; ?>">Supervisor</a></li>
            ...
            <?php 	
                ...
                elseif (isset($this->session->userdata['user_role']) && $this->session->userdata['user_role'] == 'student_supervisor_coordinator')
                { ?>
                    <ul class="nav navbar-nav navbar-center role-desktop">
                        <li class="dropdown">
                            <a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true"
                               aria-expanded="false">Switch Role <span class="caret"></span></a>
                            <ul class="dropdown-menu">
                                <li><a href="<?php echo base_url('cs/student'). '/'. $this->session->userdata['fan'] .'/'; ?>">RHD Student</a></li>
                                <li><a href="<?php echo base_url('cs/supervisor'). '/'. $this->session->userdata['fan'] .'/'; ?>">RHD Supervisor</a></li>
                                <li><a href="<?php echo base_url('cs/coordinator'). '/'. $this->session->userdata['fan'] .'/'; ?>">RHD Coordinator</a></li>
                            </ul>
                        </li>
                    </ul>
                    <ul class="nav navbar-nav navbar-right role-desktop">
            ...
            <?php
                ...
                elseif (isset($this->session->userdata['user_role']) && $this->session->userdata['user_role'] == 'supervisor_coordinator')
                { ?>
                    <ul class="nav navbar-nav navbar-center role-desktop">
                        <li class="dropdown">
                            <a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true"
            ...
            <?php
                ...
                elseif (isset($this->session->userdata['user_role']) && $this->session->userdata['user_role'] == 'student_coordinator')
                { ?>
                    <ul class="nav navbar-nav navbar-center role-desktop">
                        <li class="dropdown">
                            <a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true"
