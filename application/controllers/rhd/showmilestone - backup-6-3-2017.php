	...
	*/
    ...
    public function attach_pdf_lx9n3h2($uuid='missed', $version='missed')
    {
        ...
        if($count_current_files > 0)
        {
            ...
            $tmp_xpath_file_uuid = $xpath_files . '/version/file/uuid';
            $tmp_xpath_file_ref = $xpath_files . '/version/file/@ref';
            $tmp_xpath_file_generated = $xpath_files . '/version/file/@generated';
            ...
            {
                if($existing_attachments[$i]['uuid'] == $old_file_array['uuid']) {
                    //$existing_attachments[$i]['uuid'] = $next_previous_file_uuid;
                    $old_file_array['uuid'] = substr($old_file_array['uuid'], 0, 33) . sprintf("%03d", $previous_file_uuid_suffix);
