...
//        return;
	...
	*/
    ...
    public function attach_pdf_lx9n3h2($uuid='missed', $version='missed')
    {
        ...
        {
            $errdata['message'] = "Invalid Request";
            log_message('error', 'Attaching Milestone pdf failed, item uuid: ' . $uuid . ', error: ' . 'Invalid input params.');
            $this->load->view('rhd/showerror_view', $errdata);
            return;
        }
	 ...
	 */
        ...
        $milestone_array = $this->milestone($this->xmlwrapper);
        $milestone_array['status'] = $response['status'];
        $data = array('milestone_array' => $milestone_array);
        ...
        if($approve_date == null)
        {
            ...
            log_message('error', 'Attaching Milestone pdf failed, item uuid: ' . $uuid . ', error: ' . 'approved date is null');
            #$this->load->view('sam/showerror_view', $errdata);
            return;
        }
	...
	#log_message('error', ob_get_level ());
        ...
        {
            $errdata['message'] = $this->rexrest->error;
            log_message('error', 'Attaching Milestone pdf failed (filesCopy), item uuid: ' . $uuid . ', error: ' . $errdata['message']);
            $this->load->view('rhd/showerror_view', $errdata);
            return;
        }
        ...
        $xpath_files_first_file = $xpath_files . '/version[1]/uuid';
        
        $xpath_previous_files_first_file = $xpath_previous_files . '/version[1]/uuid';
        ...
        $count_old_files = $this->xmlwrapper->numNodes($xpath_files . '/version');
        ...
        if($count_old_files > 0)
        {
            ...
            {
                $tmp_xpath_file_uuid = $xpath_files . '/version['.$j.']/uuid';
                $tmp_xpath_file_ref = $xpath_files . '/version['.$j.']/@ref';
                $tmp_xpath_file_generated = $xpath_files . '/version['.$j.']/@generated';
        ...
        foreach ($data['milestone_array']['metadata']['availability'] as $availdata  )
        {
            ...
            $node_generated->nodeValue = $approve_date;
	...
	*/
	...
	exit();
        */
        ...
        {
            $errdata['message'] = "Item is not SAM";
            log_message('error', 'Attaching SAM pdf failed (itemIsMilestone), item uuid: ' . $uuid . ', error: ' . $errdata['message']);
            $this->load->view('sam/showerror_view', $errdata);
            return;
        }
	...
	#log_message('error', ob_get_level ());
        ...
        if($old_files_exist)
        {
            ...
            for ($j = 1; $j <= $count_old_files; $j++) 
            {
                ...
                $tmp_xpath_file_generated = $xpath_files . '/file['.$j.']/@distributed';
                 
                $tmp_file_uuid = $this->xmlwrapper->nodeValue($tmp_xpath_file_uuid);
                $tmp_file_ref = $this->xmlwrapper->nodeValue($tmp_xpath_file_ref);
                $tmp_file_distributed = $this->xmlwrapper->nodeValue($tmp_xpath_file_generated);
