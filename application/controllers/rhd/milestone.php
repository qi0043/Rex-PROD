...
class Milestone extends CI_Controller
{
     ...
     */
    public function index($format='html', $uuid='2266afd4-62eb-4d46-8ac1-a748c4564088', $version='1')
    {
        ...
        $this->logger_cs = $this->logging->get_logger('rhd'); //get cs folder
		...
		if(!$success)
		{
			...
			$this->logger_cs->error("Error: error in rest - processClientCredentialToken");
	    ...
	    $milestone_array = $this->milestoneXml2Array($this->$xmlwrapper_name);
	...
	#log_message('error', $response['status']);	
        ...
        */
        ...
        if($format=='html')
        {
            ...
            $this->load->view('rhd/showsam_view', $data);
        }
        else if($format=='pdf')
        {
     ...
     */
    protected function milestoneXml2Array($itemXml)
    {
        ...
        $milestoneArray['metadata']['studentfan'] = $itemXml->nodeValue($student_fan);
		...
		if($itemXml->numNodes('/xml/item/communication/oral/evidence/attachments/uuid') > 0)
		{
		...
		{
            $additional_education_description_text = '/xml/item/support/training/component/description/text';
            $milestoneArray['metadata']['additionaleducation']['inputs']['text'] = $itemXml->nodeValue($additional_education_description_text);
			
			
				if($itemXml->numNodes('/xml/item/support/training/component/description/attachments/uuid') > 0)
				{
        ...
        if(strnatcasecmp($milestoneArray['metadata']['additionalresource']['requested'], 'Yes') == 0) {
            ...
            for ($j = 1; $j <= $itemXml->numNodes('/xml/item/support/resources/resource/description/attachments/uuid'); $j++){
                $support_resource_input_uuid = '/xml/item/support/training/component/description/attachments/uuid['.$j.']';
        ...
        for ($j = 1; $j <= $itemXml->numNodes('/xml/item/support/resources/resource/description/attachments/uuid'); $j++){
            $research_problem_input_uuid = '/xml/item/support/resources/resource/description/attachments/uuid['.$j.']';
       ...
       $plan_proposal_input_text = '/xml/item/research/project/plan/proposal/evidence/text';
        ...
        for ($j = 1; $j <= $itemXml->numNodes('/xml/item/thesis/progress/lit_review/evidence/attachments/uuid'); $j++){
            $plan_litreview_input_uuid = '/xml/item/thesis/progress/lit_review/evidence/evidence/attachments/uuid['.$j.']';
        ...
        $milestoneArray['metadata']['planproposal']['comment'] = $itemXml->nodeValue($timeframe_comment);
        ...
        for ($j = 1; $j <= $itemXml->numNodes('/xml/item/progression/goals/agreed'); $j++)
        {
            ...
            $goal_content = '/xml/item/progression/goals/agreed['.$j.']/goal';

            $milestoneArray['metadata']['agreedgoal'][$j]['timeframe'] = $itemXml->nodeValue($time_frame);
            $milestoneArray['metadata']['agreedgoal'][$j]['goalcontent'] = $itemXml->nodeValue($goal_content);
        }
        ...
        $mod_coordinator_comment = '/xml/item/progression/outcome/recommendation/comment';
